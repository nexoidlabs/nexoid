...
describe("AllowanceModule", function () {
  async function deployFixture() {
    ...
    // Mint USDT to the Safe
    const INITIAL_BALANCE = 10000n * 10n ** 6n; // 10,000 USDT
    await usdt.mint(safe.address, INITIAL_BALANCE);

    // Safe approves AllowanceModule to transfer USDT (simulates module being enabled)
    await usdt.connect(safe).approve(await module.getAddress(), INITIAL_BALANCE);
  ...
  describe("Allowance Management", function () {
    it("should set an allowance for a delegate", async function () {
      const { module, usdt, safe, delegate } = await loadFixture(deployFixture);
      ...
      await expect(
        ...
        .withArgs(safe.address, delegate.address, await usdt.getAddress(), amount, 0);
    });
    ...
    it("should query allowance via getTokenAllowance", async function () {
      ...
      await module
        .connect(safe)
        .setAllowance(delegate.address, await usdt.getAddress(), amount, 1440, 0);
    ...
    it("should reject setting allowance for non-delegate", async function () {
      const { module, usdt, safe, stranger } = await loadFixture(deployFixture);

      await expect(
        module
          .connect(safe)
          .setAllowance(stranger.address, await usdt.getAddress(), 100n, 0, 0)
      ).to.be.revertedWith("Not a delegate");
    });
    ...
    it("should track tokens for a delegate", async function () {
      ...
      await module
        .connect(safe)
        .setAllowance(delegate.address, await usdt.getAddress(), 100n * 10n ** 6n, 0, 0);

      const trackedTokens = await module.getTokens(safe.address, delegate.address);
      expect(trackedTokens).to.include(await usdt.getAddress());
    });
    ...
    it("should delete allowance", async function () {
      ...
      await module
        .connect(safe)
        .deleteAllowance(delegate.address, await usdt.getAddress());
    ...
    it("should reset spent amount", async function () {
      const { module, usdt, safe, delegate, recipient } = await loadFixture(deployFixture);

      await module.connect(safe).addDelegate(delegate.address);
      const amount = 100n * 10n ** 6n;
      await module
        .connect(safe)
        .setAllowance(delegate.address, await usdt.getAddress(), amount, 0, 0);
      ...
      await module
        .connect(safe)
        .resetAllowance(delegate.address, await usdt.getAddress());
  ...
  describe("Transfer Execution", function () {
    it("should execute allowance transfer from delegate", async function () {
      ...
      await module
        .connect(safe)
        .setAllowance(delegate.address, await usdt.getAddress(), allowanceAmount, 0, 0);

      const transferAmount = 25n * 10n ** 6n; // 25 USDT
      ...
      expect(await usdt.balanceOf(safe.address)).to.equal(10000n * 10n ** 6n - transferAmount);
    ...
    it("should reject transfer exceeding allowance", async function () {
      ...
      await module
        .connect(safe)
        .setAllowance(delegate.address, await usdt.getAddress(), 50n * 10n ** 6n, 0, 0);
      ...
      await expect(
        module.connect(delegate).executeAllowanceTransfer(
          ...
          await usdt.getAddress(),
          recipient.address,
          51n * 10n ** 6n, // 51 USDT > 50 allowance
          hre.ethers.ZeroAddress,
          0,
          delegate.address,
          "0x"
    ...
    it("should reject transfer from non-delegate caller", async function () {
      const { module, usdt, safe, delegate, stranger, recipient } =
    ...
    it("should allow multiple transfers within allowance", async function () {
      ...
      // Transfer 3: 31 USDT — should fail (30+40+31 > 100)
      await expect(
        module.connect(delegate).executeAllowanceTransfer(
          safe.address,
          await usdt.getAddress(),
      ...
      // Transfer 3b: 30 USDT — should succeed (30+40+30 = 100)
      await module.connect(delegate).executeAllowanceTransfer(
        safe.address,
        await usdt.getAddress(),
        recipient.address,
      ...
      expect(await usdt.balanceOf(recipient.address)).to.equal(100n * 10n ** 6n);
    ...
    it("should support multiple delegates independently", async function () {
      const { module, usdt, safe, delegate, delegate2, recipient } =
        await loadFixture(deployFixture);
      ...
      await module
        .connect(safe)
        .setAllowance(delegate2.address, await usdt.getAddress(), 200n * 10n ** 6n, 0, 0);
      ...
      expect(await usdt.balanceOf(recipient.address)).to.equal(230n * 10n ** 6n);
    });
  });
  ...
  describe("Hash Generation", function () {
    ...
    it("should produce different hash for different amount", async function () {
      const { module, usdt, safe, recipient } = await loadFixture(deployFixture);
  ...
  describe("Edge Cases", function () {
    ...
    it("should handle zero-amount allowance", async function () {
      ...
      await module
        .connect(safe)
        .setAllowance(delegate.address, await usdt.getAddress(), 0, 0, 0);
    ...
    it("should update allowance amount", async function () {
      ...
      await module
        .connect(safe)
        .setAllowance(delegate.address, await usdt.getAddress(), 500n * 10n ** 6n, 0, 0);
